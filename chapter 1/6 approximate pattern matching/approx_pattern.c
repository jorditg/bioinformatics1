...
void approx_pattern_match(char * genome, size_t genome_len, char * pattern, size_t pattern_len, int allowed_mismatches);
...
int main(int argc, char *argv[])
{
	...
	char * pattern = argv[2];
	int allowed_mismatches = atoi(argv[3]);
	
	size_t pattern_len = strlen(pattern);
	
	//printf("genome_len=%i, pattern=%s, pattern_len=%i\n", genome_len, pattern, pattern_len);
	approx_pattern_match(genome, genome_len, pattern, pattern_len, allowed_mismatches);
}
