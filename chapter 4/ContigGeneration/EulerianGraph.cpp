...
int EulerianGraph::nr_predecessors(int node)
{
    int result = 0;
    
    std::map<int, std::list<int> >::iterator it_m = graph.begin();    
    while(it_m != graph.end()) {
        // check if it has none ore more than one predecessor
        std::list<int> &l = it_m->second;
        std::list<int>::iterator it = std::find(l.begin(), l.end(), node); 
        if(it != l.end()) result++;
        it_m++;
    }
    return result;
...
void EulerianGraph::print(const std::list<int> &l)
{
