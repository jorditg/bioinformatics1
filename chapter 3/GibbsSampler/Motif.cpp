...
void Motif::calculate_probabilities()
{
    prob.clear();
    int k = m[0].size();
    //               A   C   G   T
    std::array<float, 4> a = {0.0,0.0,0.0,0.0};
    prob.resize(k, a);
    for(int j = 0;j<k;j++) {
...
void Motif::calculate_probabilities_with_pseudocounts_without(int index)
{
    prob.clear();
    int k = m[0].size();
    //               A   C   G   T
