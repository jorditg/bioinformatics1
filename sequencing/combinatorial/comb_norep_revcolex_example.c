...
testing
gen_comb_norep_revcolex_init() and gen_comb_norep_revcolex_next()

compile
gcc -o comb_norep_revcolex_example comb_norep_revcolex_example.c comb_norep_revcolex.c
*/
...
set_counter = 0;
printf("comb_norep_revcolex(%u, %u)\n", n, k);

//initialize
gen_result = gen_comb_norep_revcolex_init(vector, n, k);
...
while(gen_result == GEN_NEXT)
 ...
 gen_result = gen_comb_norep_revcolex_next(vector, k);
 }
