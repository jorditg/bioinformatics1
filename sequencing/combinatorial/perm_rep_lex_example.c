...
testing
gen_perm_rep_lex_init() and gen_perm_rep_lex_next()

compile
gcc -o perm_rep_lex_example perm_rep_lex_example.c perm_rep_lex.c
*/
...
set_counter = 0;
printf("perm_rep_lex(%u)\n", n);

//initialize
gen_result = gen_perm_rep_lex_init(n);
...
while(gen_result == GEN_NEXT)
 ...
 gen_result = gen_perm_rep_lex_next(vector, n);
 }
